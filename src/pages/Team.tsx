...
export default function Team() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-5 md:px-10 py-[50px] lg:py-[100px]">
      ...
      <div className="mb-32">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-x-6 gap-y-12">
          {loading ? (
            ...
            team.filter(m => m.role === 'Member').map((member, i) => (
              <motion.div
                key={member._id}
                initial={{ opacity: 0, scale: 0.95 }}
                whileInView={{ opacity: 1, scale: 1 }}
                  ...
                  <div className="absolute inset-x-0 top-0 aspect-[4/5] pointer-events-none flex flex-col justify-end overflow-hidden z-20">
                    <div className="p-6 translate-y-full group-hover:translate-y-0 transition-transform duration-500 bg-gradient-to-t from-bg/90 to-transparent pointer-events-auto">
                      <div className="flex justify-center gap-3">
                        {member.linkedin && <a href={member.linkedin} target="_blank" rel="noopener noreferrer" className="w-9 h-9 glass rounded-full flex items-center justify-center hover:bg-primary text-white/70 hover:text-white transition-all transform hover:scale-110"><Linkedin size={14} /></a>}
                        {member.facebook && <a href={member.facebook} target="_blank" rel="noopener noreferrer" className="w-9 h-9 glass rounded-full flex items-center justify-center hover:bg-primary text-white/70 hover:text-white transition-all transform hover:scale-110"><Facebook size={14} /></a>}
                        {member.instagram && <a href={member.instagram} target="_blank" rel="noopener noreferrer" className="w-9 h-9 glass rounded-full flex items-center justify-center hover:bg-primary text-white/70 hover:text-white transition-all transform hover:scale-110"><Instagram size={14} /></a>}
                        {member.telegram && <a href={`https://t.me/${member.telegram.replace('@', '')}`} target="_blank" rel="noopener noreferrer" className="w-9 h-9 glass rounded-full flex items-center justify-center hover:bg-primary text-white/70 hover:text-white transition-all transform hover:scale-110"><TelegramIcon size={14} /></a>}
                        {!member.linkedin && !member.facebook && !member.instagram && !member.telegram && <div className="w-9 h-9 glass rounded-full flex items-center justify-center text-white/20"><Globe size={14} /></div>}
                      </div>
                    </div>
                  </div>
                </div>
      ...
      {!loading && team.filter(m => m.role === 'User' || m.role === 'Viewer').length > 0 && (
        <div className="mt-32">
          <div className="flex flex-col items-center mb-16 text-center">
            <motion.div
              initial={{ opacity: 0 }}
              whileInView={{ opacity: 1 }}
              className="text-primary font-mono text-[10px] uppercase tracking-[0.3em] mb-4"
            >
              LEGACY EXPERTS
            </motion.div>
            <h2 className="text-3xl md:text-5xl font-display font-bold">Our Previous <span className="text-white/40 italic">Ex-Members</span></h2>
            <p className="text-white/40 max-w-xl mx-auto mt-6 text-sm">
              Honoring the brilliant minds who have contributed to the EleSquad journey and helped build our digital legacy.
            </p>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {team.filter(m => m.role === 'User' || m.role === 'Viewer').map((member, i) => (
              <motion.div 
                key={member._id}
                initial={{ opacity: 0, y: 20 }}
                ...
                viewport={{ once: true }}
                transition={{ delay: i * 0.1 }}
                className="glass p-8 rounded-[2.5rem] border border-white/5 hover:border-primary/30 transition-all group text-center"
              >
                 <div className="w-32 h-32 rounded-[2rem] bg-white/5 border border-white/10 flex items-center justify-center overflow-hidden mx-auto mb-6 transition-all duration-500 group-hover:scale-105 group-hover:shadow-[0_0_20px_rgba(108,77,246,0.2)]">
                    {member.image ? (
                      <img src={member.image} className="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-700" />
                    ) : (
                      <span className="text-3xl font-bold text-white/10 group-hover:text-primary transition-colors">{member.name[0]}</span>
                    )}
                 </div>
                 <h4 className="text-xl font-display font-bold text-white/60 group-hover:text-white transition-colors">{member.name}</h4>
                 <p className="text-[9px] font-mono uppercase tracking-[0.2em] text-primary/40 group-hover:text-primary transition-colors mt-2">
                   {member.designation || 'Legacy Expert'}
                 </p>
              </motion.div>
            ))}
          </div>
